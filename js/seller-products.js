(() => {
  ...
  function isSellerLoggedIn() {
    ...
    return loggedIn && (!loginType || loginType === "seller");
  }
  ...
  function moveToSellerLogin() {
    const redirect = encodeURIComponent("seller-products.html");
    location.replace(`login.html?type=seller&redirect=${redirect}`);
  }
  ...
  async function logout() {
    try {
      ...
      clearLoginState();
      location.href = "login.html?type=seller";
    }
  }
  ...
  function normalizeProduct(raw) {
    ...
    const stock = raw.stock ?? raw.stockQuantity ?? raw.quantity ?? 0;
    return {
      productId,
      ...
      price: raw.salePrice ?? raw.discountPrice ?? raw.price ?? 0,
      stock,
      status: normalizeStatus(raw.status ?? raw.salesStatus, stock),
  ...
  function extractPageData(data) {
    ...
    const rawItems = body.content ?? body.items ?? body.products ?? body.list ?? (Array.isArray(body) ? body : []);
      ...
      <tr>
        <td>
          <div class="product-info">
            ${product.imageUrl ? `<img class="product-thumb" src="${escapeHtml(product.imageUrl)}" alt="">` : `<div class="product-thumb" aria-hidden="true"></div>`}
        ...
        <td>
          <div class="manage-buttons">
            <a href="seller-product-edit.html?id=${encodeURIComponent(product.productId)}">수정</a>
            <button class="delete-button" type="button" data-delete-id="${escapeHtml(product.productId)}" data-delete-name="${escapeHtml(product.productName)}">삭제</button>
          </div>
        </td>
      </tr>
    `).join("");
    ...
    mobileList.innerHTML = products.map((product) => `
      <article class="mobile-product-card">
        ...
        <div class="mobile-card-body">
          <div class="mobile-card-head"><strong>${escapeHtml(product.productName)}</strong>${statusBadge(product.status)}</div>
          <div class="mobile-meta">
            <span>상품번호</span><b>${escapeHtml(product.productId)}</b>
            <span>판매가</span><b>${formatPrice(product.price)}</b>
            <span>재고</span><b>${Number(product.stock).toLocaleString("ko-KR")}</b>
          </div>
          <div class="manage-buttons">
            <a href="seller-product-edit.html?id=${encodeURIComponent(product.productId)}">수정</a>
            <button class="delete-button" type="button" data-delete-id="${escapeHtml(product.productId)}" data-delete-name="${escapeHtml(product.productName)}">삭제</button>
  ...
  function renderPagination() {
    ...
    prev.addEventListener("click", () => loadProducts(currentPage - 1));
  ...
  async function loadProducts(page = 0) {
    ...
    try {
      ...
      if (!response.ok) throw new Error(data.message || "상품 목록을 불러오지 못했습니다.");
      ...
      showMessage(error instanceof TypeError ? "서버에 연결할 수 없습니다. WEB/WAS 서버 상태를 확인해 주세요." : error.message);
    }
  }
  ...
  async function deleteProduct() {
    ...
    try {
      const response = await fetch(`${API_URL}/${encodeURIComponent(selectedProduct.productId)}`, {
        method: "DELETE",
        credentials: "include"
      });
      ...
      let data = {};
      try { data = await response.json(); } catch (_) {}
      if (!response.ok) throw new Error(data.message || "상품 삭제에 실패했습니다.");
